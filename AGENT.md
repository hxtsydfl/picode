...
# Run daemon manually
uv run picode-core                        # foreground; Ctrl+C to stop
PICODE_PORT=8000 uv run picode-core        # override port
...
This is a **dual-process** local AI agent system. `picode-core` is a persistent daemon; `picode` and `picode-tui` are clients that connect to it over a Unix domain socket.

```
picode-core (daemon)
  └─ listens on 127.0.0.1:7437 (TCP)
       ↑ JSON-RPC 2.0 NDJSON
picode (CLI)   picode-tui (TUI, S2+)
```

**`picode-tui` is the primary frontend.** All user-facing work on task management, observability, and interaction should be designed for and validated in the TUI first. The `picode` CLI exists only for quick scripted testing and debugging — it is not a product surface. When implementing features that touch the user interface, invest in the TUI layout, event rendering, and keyboard interactions. Do not shortcut TUI work by pointing to the CLI as an alternative.

### Protocol layer (`src/picode/core/bus/`)
...
### Transport layer (`src/picode/core/transport/`)
...
### Config (`src/picode/core/config.py`)

Four-tier priority: **built-in defaults → `~/.picode/config.toml` → `.env` → env vars**.
...
Relevant env vars: `PICODE_CONFIG`, `PICODE_HOST`, `PICODE_PORT`, `PICODE_LOG_LEVEL`, `PICODE_LOG_FILE`, `PICODE_LOG_FORMAT`.

### Daemon entry (`src/picode/core/app.py`)
...
Integration tests in `tests/conftest.py` spawn a real daemon subprocess using a random free port (via `free_port` fixture). The fixture finds a free port, releases it, passes it to the daemon via `PICODE_PORT`, then polls `asyncio.open_connection` until the daemon is ready.
