...
def test_dotenv_base_loaded(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _write_env(env_file, "PICODE_PORT=9999\n")
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("PICODE_PORT", raising=False)
...
def test_system_env_overrides_dotenv(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _write_env(env_file, "PICODE_PORT=9999\n")
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PICODE_PORT", "8888")
...
# 功能：验证 .env 中设置的 PICODE_CONFIG 能正确影响 TOML 配置文件的加载路径
...
def test_dotenv_before_toml_kama_config(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    env_file = tmp_path / ".env"
    _write_env(env_file, f"PICODE_CONFIG={toml_path}\n")
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("PICODE_CONFIG", raising=False)
    monkeypatch.delenv("PICODE_PORT", raising=False)
...
def test_priority_chain_full(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # .env：7000
    # 系统环境变量：8000（最高）
    toml_path = tmp_path / "picode.toml"
    toml_path.write_bytes(b'[core]\nport = 6000\n')

    env_file = tmp_path / ".env"
    _write_env(env_file, "PICODE_PORT=7000\n")

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PICODE_CONFIG", str(toml_path))
    monkeypatch.setenv("PICODE_PORT", "8000")
